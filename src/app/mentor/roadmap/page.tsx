...
export default function RoadmapPage() {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {/* Header Title */}
      <div>
        <h1 className="text-3xl font-bold font-sans text-slate-900 dark:text-white tracking-tight">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-emerald-50/70 dark:bg-emerald-950/40 p-5 rounded-2xl border border-emerald-200/60 dark:border-emerald-900/60 shadow-2xs space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-gray-200/80 dark:border-slate-800 shadow-2xs space-y-8">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Milestones</h2>

          <div className="space-y-8 relative before:absolute before:left-4 before:top-3 before:bottom-3 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
            {milestones.map((item) => {
              ...
              return (
                <div key={item.step} className="relative pl-10 space-y-2 group">
                  ...
                  <div
                    className={`absolute left-0 top-0.5 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-transform group-hover:scale-110 ${
                      isCompleted
                        ...
                        : isInProgress
                        ? "bg-[#3b28cc] dark:bg-purple-600 text-white ring-4 ring-purple-100 dark:ring-purple-950"
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-400 dark:text-slate-400">Step {item.step} • {item.status}</span>
                  </div>

                  {/* Title & Description */}
                  <h3 className={`text-base font-bold ${isInProgress ? "text-[#3b28cc] dark:text-purple-400" : "text-slate-900 dark:text-white"}`}>
                    {item.title}
                  </h3>
                  ...
                  {item.resource && (
                    <div className="mt-3 p-4 rounded-xl bg-cyan-50/70 dark:bg-cyan-950/40 border border-cyan-200/60 dark:border-cyan-900/60 flex items-center justify-between gap-4">
                      <div>
                        <span className="text-[10px] font-bold text-cyan-800 dark:text-cyan-300 uppercase tracking-wider block">
                          Recommended Resource
                        </span>
                        <h4 className="font-bold text-slate-800 dark:text-slate-200 text-xs mt-0.5">{item.resource.title}</h4>
                      </div>
                      <a
                        href={item.resource.link}
                        className="text-xs font-semibold text-[#3b28cc] dark:text-purple-400 hover:underline shrink-0 flex items-center gap-1"
                      >
        ...
        <div className="lg:col-span-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-200/80 dark:border-slate-800 shadow-2xs space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-400 mt-1">
              Resources aligned with your current focus: <span className="font-semibold text-slate-700 dark:text-slate-300">Design Systems Architecture</span>.
            </p>
          </div>
          ...
          <div className="space-y-3">
            {curatedResources.map((res, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-3.5 rounded-xl border border-slate-100 dark:border-slate-800 hover:border-gray-200 dark:hover:border-slate-700 hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-all flex items-start gap-3"
                >
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 dark:text-slate-200 text-xs">{res.title}</h4>
                    <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{res.type}</p>
                  </div>
                </div>
              );
            })}
