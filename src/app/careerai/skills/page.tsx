...
export default function SkillsPage() {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold font-sans text-slate-900 dark:text-white tracking-tight">AI Skill Gap Analysis</h1>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-gray-200/80 dark:border-slate-800 shadow-2xs space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">Verified Skills Matrix</h2>
        ...
        <div className="space-y-4">
          {skillsMatrix.map((item, idx) => (
            <div key={idx} className="p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/50 space-y-2">
              <div className="flex items-center justify-between text-xs sm:text-sm">
                <span className="font-bold text-slate-800 dark:text-slate-200">{item.skill}</span>
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 dark:text-slate-400 text-xs">Market Demand: <strong className="text-slate-700 dark:text-slate-200">{item.demand}</strong></span>
